...
class CarbonTracker:
    ...
    def __init__(self, Global, year_start_for_PDV=0):
        ...
        self.product_LLP_pool_secondary, self.product_SLP_pool_secondary, self.product_VSLP_pool_secondary = [np.zeros((self.Global.ncycles_harvest, self.Global.arraylength)) for _ in range(3)]
        ### Slash pool
    ...
    def carbon_pool_simulator_per_cycle(self):
        ...
        for cycle in range(0, self.Global.ncycles_harvest):
            ...
            if self.Global.year_index_both_plantation[cycle] in self.Global.year_index_harvest_plantation:
                ...
                self.product_VSLP_pool_secondary[cycle, year_harvest_thinning] = aboveground_biomass_before_harvest * self.Global.harvest_percentage_plantation[year_harvest_thinning] * self.product_share_VSLP_secondary[year_harvest_thinning - 1]
                ...
                self.product_VSLP_pool_secondary[cycle, year_harvest_thinning] = aboveground_biomass_before_harvest * self.Global.harvest_percentage_plantation[year_harvest_thinning] * self.Global.product_share_VSLP_thinning
                self.slash_pool_secondary[cycle, year_harvest_thinning] = aboveground_biomass_before_harvest * self.Global.harvest_percentage_plantation[year_harvest_thinning] * self.Global.slash_percentage_secondary_conversion[year_harvest_thinning]
