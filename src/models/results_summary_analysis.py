...
# FIXME WFL vs IND, I need to consider breakdown the plantation into the two, don't exclude plantation
# FIXME run the all discounting rate scenario
...
def barplot_all_scenarios():
    ...
    def stacked_barplot_attribute_demand_substitution_land(result_df, title, ylabel):
        ...
        result_df_secondary.loc['New plantations', 'S4 New tropical plantations'] = result_df.loc['BAU_NOSUB_ALL', 'New plantations']
    ...
    def stacked_barplot_attribute_IND_WFL_carbon(result_df, title, ylabel):
        ...
        # x ticks
        xticks_labels = ['S1 Secondary forest\nHarvest + Regrowth', 'S2 Secondary forest\nHarvest + Conversion', 'S3 Secondary forest\nMixed harvest',
     'S4 Higher\nPlantation productivity', 'S5 50% Less wood fuel\n2050 Demand', 'S6 New\nTropical plantations']
        plt.xticks(result_df.columns, labels=xticks_labels)
        # bar labels
    ...
    def stacked_barplot_attribute_IND_WFL_land(result_df, title, ylabel):
        ...
        plt.bar(result_df_secondary.columns, result_df_secondary.loc['BAU_SUBON_ALL']*result_df_secondary.loc['BAU_SUBON_IND']/(result_df_secondary.loc['BAU_SUBON_IND']+result_df_secondary.loc['BAU_SUBON_WFL']), bottom=result_df_secondary.loc['Existing plantations'], color='#ab5b1a', width=0.5)
        plt.bar(result_df_secondary.columns, result_df_secondary.loc['BAU_SUBON_ALL']*result_df_secondary.loc['BAU_SUBON_WFL']/(result_df_secondary.loc['BAU_SUBON_IND']+result_df_secondary.loc['BAU_SUBON_WFL']), bottom=result_df_secondary.loc['Existing plantations']+result_df_secondary.loc['BAU_SUBON_ALL']*result_df_secondary.loc['BAU_SUBON_IND']/(result_df_secondary.loc['BAU_SUBON_IND']+result_df_secondary.loc['BAU_SUBON_WFL']), color='#d48f57', width=0.5)
        plt.bar(result_df_secondary.columns, result_df_secondary.loc['New plantations'], bottom=result_df_secondary.loc['Existing plantations']+result_df_secondary.loc['BAU_SUBON_ALL'], edgecolor='k', color='#f59e1b', width=0.5)
        plt.bar(result_df_secondary.columns, result_df_secondary.loc['BAU_SUBON_ALL'], bottom=result_df_secondary.loc['Existing plantations'], ls='dashed', facecolor="None", edgecolor='k', width=0.5)
        # x and y limits
        # plt.xlim(-0.6, 10.5)
        # plt.ylim(-1600, 2000)
     ...
     'S4 Higher\nPlantation productivity', 'S5 50% less wood fuel\n2050 Demand', 'S6 New\nTropical plantations']
        ...
        ax.text(ax.patches[21].get_x() + ax.patches[21].get_width() / 2, ax.patches[21].get_y() + ax.patches[21].get_height() / 2,
                ...
                ha='center', va='bottom')
        # Add the final absolute numbers at the top
        area_total = result_df_secondary.loc['Existing plantations'] + result_df_secondary.loc['BAU_SUBON_ALL'] + result_df_secondary.loc['New plantations']
        for number, rec in enumerate(ax.patches[:6]):
            # This is to make sure the turn of the group of scenario
            ax.text(rec.get_x() + rec.get_width() / 2, rec.get_y() + area_total[number],
                    "{:.0f}".format(area_total[number]),
        ...
        # title and legend
        legend_label = ['Existing plantations', 'Industrial roundwood', 'Wood fuel', 'New tropical plantations', 'Total secondary forest area']
        plt.legend(legend_label, ncol=2, bbox_to_anchor=([1, 1.15, 0, 0]), frameon=False, fontsize=14)
